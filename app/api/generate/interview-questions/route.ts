...
import { generateInterviewQuestions } from "@/lib/generateAssets";
...
export async function POST(req: Request) {
  try {
    ...
    try {
      const result = await generateInterviewQuestions(extractedText, combinedJdText);
      return NextResponse.json(result);
    } catch (error: any) {
      console.error("Gemini Generation Error:", error);
      let errorMsg = "Failed to generate Interview Questions.";
